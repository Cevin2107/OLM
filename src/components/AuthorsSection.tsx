  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Form tabs */}
        <div className="flex gap-1 border-b border-[#3b82c4]/30 mb-5">
          {([['basic', 'Cơ Bản'], ['detail', 'Chi Tiết'], ['media', 'Tư Liệu']] as const).map(([key, label]) => (
            <button key={key} onClick={() => setFormTab(key)}
              className={`px-4 py-2 text-sm font-semibold transition-colors border-b-2 -mb-px ${formTab === key ? 'border-[#3b82c4] text-[#1a4f99]' : 'border-transparent text-[#1c1c1c]/60 hover:text-[#1c1c1c]'}`}>
              {label}
            </button>
          ))}
        </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="flex items-start gap-4">
                <div className="w-24 h-24 rounded-lg overflow-hidden bg-[#1b2d47] flex-shrink-0 flex items-center justify-center border-2 border-[#3b82c4]/30">
                  {uploading ? <Loader2 size={24} className="text-[#3b82c4] animate-spin" />
                    : form.avatar_url ? <img src={form.avatar_url} alt="avatar" className="w-full h-full object-cover" />
                    : <User size={32} className="text-[#3b82c4]/50" strokeWidth={1} />}
                </div>
            ...
            <div>
              <label className="block text-sm font-semibold text-[#1c1c1c] mb-1">Tên tác giả <span className="text-[#1a4f99]">*</span></label>
              <input className="w-full border border-[#3b82c4]/40 rounded px-3 py-2 bg-white text-[#1c1c1c] focus:outline-none focus:border-[#3b82c4]"
            ...
            <div>
              ...
              <textarea className="w-full border border-[#3b82c4]/40 rounded px-3 py-2 bg-white text-[#1c1c1c] focus:outline-none focus:border-[#3b82c4] min-h-[80px] resize-y"
        ...
        {formTab === 'detail' && (
          <div className="space-y-4">
            <div>
              ...
              <textarea className="w-full border border-[#3b82c4]/40 rounded px-3 py-2 bg-white text-[#1c1c1c] focus:outline-none focus:border-[#3b82c4] min-h-[160px] resize-y"
    ...
    >
      ...
      >
        ...
        <div className="flex flex-col md:flex-row rounded-t-2xl overflow-hidden">
          <div className="md:w-2/5 bg-[#1b2d47] flex items-center justify-center min-h-[240px] md:min-h-[300px] relative flex-shrink-0">
            {author.avatar_url ? (
              <img src={author.avatar_url} alt={author.name} className="w-full h-full object-cover absolute inset-0" />
            ) : (
              <User className="w-28 h-28 text-[#3b82c4]" strokeWidth={1} />
            )}
          </div>
          <div className="flex-1 p-4 md:p-8 flex flex-col justify-center">
            <div className="flex items-start justify-between gap-3 mb-3">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#1a4f99] mb-1">Tác Giả</p>
                <h2 className="text-3xl md:text-4xl font-serif font-bold text-[#1c1c1c] leading-tight tracking-wide">
                  {author.name}
                </h2>
                {author.birth_death_year && (
                  <p className="text-[#1a4f99] font-semibold mt-2 tracking-wider">{author.birth_death_year}</p>
                )}
                {author.hometown && (
                  <p className="text-[#1c1c1c]/60 text-sm mt-1 flex items-center gap-1.5">
                    <MapPin size={13} className="text-[#3b82c4]" />
                    {author.hometown}
                  </p>
                )}
              </div>
              <button onClick={onClose}
                className="w-8 h-8 rounded-full bg-[#1b2d47]/10 flex items-center justify-center hover:bg-[#1b2d47]/20 transition-colors flex-shrink-0 mt-1">
                <X size={16} className="text-[#1c1c1c]" />
              </button>
            </div>
            {author.bio_short && (
              <>
                <div className="w-16 h-px bg-[#3b82c4] mb-4" />
        ...
        {/* ── Tabs ── */}
        <div className="border-t border-[#3b82c4]/20 px-4 md:px-8 pt-5">
          <div className="flex gap-1 border-b border-[#3b82c4]/20 mb-6 overflow-x-auto">
            {TABS.map((t) => (
              <button key={t.key} onClick={() => setActiveTab(t.key)}
                className={`px-5 py-2.5 text-sm font-semibold whitespace-nowrap transition-colors border-b-2 -mb-px ${
                  activeTab === t.key ? 'border-[#3b82c4] text-[#1a4f99]' : 'border-transparent text-[#1c1c1c]/60 hover:text-[#1c1c1c]'
                }`}>
                {t.label}
              </button>
            ))}
          ...
          {activeTab === 'life' && (
            <div className="pb-8 min-h-[160px]">
              {author.bio_life ? (
                <MarkdownText text={author.bio_life} />
              ) : (
          ...
          {activeTab === 'style' && (
            <div className="pb-8 min-h-[160px]">
              {author.bio_style ? (
                <MarkdownText text={author.bio_style} />
              ) : (
                ...
                <div className="flex gap-3 overflow-x-auto pb-2">
                  {works.map((w) => (
                    <button key={w.id}
                      ...
                      className="flex-shrink-0 w-44 p-4 rounded-xl border-2 text-left transition-all duration-200 border-[#3b82c4]/30 bg-white text-[#1c1c1c] hover:border-[#3b82c4] hover:shadow">
                      <p className="font-serif font-bold text-sm leading-snug line-clamp-2">{w.title}</p>
                      {w.genre && (
                        <p className="text-xs mt-1.5 font-medium text-[#1a4f99]">
                          {w.genre.name}
                        </p>
                      )}
                      {w.composition_year && (
                        <p className="text-xs mt-0.5 text-[#1c1c1c]/50">
                          {w.composition_year}
                        </p>
                      )}
                    </button>
          ...
          {activeTab === 'media' && (
            <div className="pb-8 min-h-[160px]">
              {author.video_doc_id ? (
                <>
                  <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#1a4f99] mb-4">Video Tư Liệu</p>
        ...
        <div className="px-4 md:px-8 pb-6 md:pb-8">
          <button
            onClick={onClose}
            className="w-full px-8 py-3 bg-[#1b2d47] text-[#ccd8ef] uppercase tracking-widest text-sm font-semibold rounded-lg hover:bg-[#3b82c4] hover:text-[#1c1c1c] transition-colors duration-300"
          >
            Đóng
          </button>
        </div>
      ...
      {/* Work detail popup – rendered inside backdrop so it layers above */}
      {selectedWork && (
...
function AuthorTooltip({ author, pos }: { author: Author; pos: { x: number; y: number } }) {
  ...
  return (
    <div style={style}>
      <div className="w-56 bg-[#1a1a1a]/95 backdrop-blur-sm border border-[#3b82c4]/30 rounded-xl shadow-2xl overflow-hidden animate-[fadeIn_0.15s_ease]">
        {author.avatar_url ? (
          <div className="h-32 overflow-hidden">
            <img src={author.avatar_url} alt={author.name} className="w-full h-full object-cover" />
          </div>
        ) : (
          <div className="h-24 bg-[#1b2d47] flex items-center justify-center">
            <User size={36} className="text-[#3b82c4]" strokeWidth={1} />
          </div>
        )}
        <div className="p-3">
          <p className="font-serif font-bold text-[#ccd8ef] leading-tight">{author.name}</p>
          {author.birth_death_year && (
            <p className="text-[#3b82c4] text-xs mt-0.5 font-medium">{author.birth_death_year}</p>
          )}
          {author.bio_short && (
            <p className="text-[#ccd8ef]/60 text-xs mt-1.5 leading-relaxed line-clamp-2">
              {author.bio_short}
            </p>
          )}
          <p className="text-[#3b82c4]/50 text-xs mt-2 italic">Nhấn để xem chi tiết</p>
        </div>
      </div>
    </div>
  );
...
function AuthorCard({
  ...
  return (
    <>
      <div
        ref={cardRef}
        className="spotlight-hover bg-[#eff4fc] rounded-lg overflow-hidden relative group cursor-pointer"
      ...
      >
        ...
        <div className="aspect-[3/4] bg-[#1b2d47] flex items-center justify-center overflow-hidden">
          {author.avatar_url ? (
            ...
            <User className="w-16 h-16 text-[#3b82c4]" strokeWidth={1} />
          )}
        </div>
...
export function AuthorsSection() {
  ...
  return (
    <section id="authors" className="py-20 px-4 bg-gradient-to-b from-[#1a1a1a] to-[#1b2d47]">
        ...
        {authors.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-[#ccd8ef]/60 text-lg">
              ...
              {isEditMode && (
                <button onClick={() => setModalAuthor('new')} className="text-[#3b82c4] underline">
